...
export const DiscountStyleSchema = z.object({
  ...
  text_color: z.string(),
  font_size: z.number(),
  animation: z.number().nullable().default(0),
});
...
export const DiscountBannerTemplateSchema = z.object({
  template: z.literal(0),
  banner_text: z.string(),
  content_order: z.string().nullable().default("text,coupon,action"),
...
export const DiscountBannerSchema = z.object({
  ...
  banner_templates: z.array(DiscountBannerTemplateSchema).length(1),
});

export type DiscountBanner = z.infer<typeof DiscountBannerSchema>;
